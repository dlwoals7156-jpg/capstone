...
from backend.app.config import PRODUCT_SQL_PATH
...
SKELETON_KEYS = {"스트레이트": "straight", "웨이브": "wave", "내추럴": "natural"}
BODY_KEYS = {"역삼각형": "inverted_triangle", "삼각형": "pear", "직사각형": "rectangle", "모래시계": "hourglass", "타원형": "oval"}
...
def build_style_note(request: RecommendRequest, products: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            f"'{query}' 요청은 {situation_text} 상황에서 바로 사용할 수 있는 후보를 먼저 봤어요. "
            f"{personal_color_label}, {request.skeleton_type}, {request.body_shape} 정보를 함께 놓고 보니 "
            f"'{top_item}'이 가장 자연스럽습니다. 스타일은 {style_text} 흐름을 살리되 실제로 고르기 쉬운 상품을 앞쪽에 배치했어요."
        ),
    }
